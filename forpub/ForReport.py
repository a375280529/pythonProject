...
class ForReport(ReportTestResult,PATH):
    img_path = 'img/' if platform.system() != 'Windows' else 'img\\'
    
    def __init__(self, suites):
        super(ForReport, self).__init__(suites)
        self.suites = suites
        self.log_path = None
        self.title = '自动化测试报告'
    ...
    def add_test_img(*pargs):
        ...
        def _wrap(func):
            ...
            def __wrap(*args, **kwargs):
                #img_path = os.path.abspath('{}'.format(ForReport.img_path))
                img_path = "E:/test/baogao/Test_Report/img"
                try:
                    ...
                    if 'save_img' in dir(args[0]):
                        ...
                        data = ForReport.img2base(img_path, pargs[0] + '.png')
                ...
                if len(pargs) > 1:
                    for parg in pargs:
                        print(parg + ':')
                        data = ForReport.img2base(img_path, parg + '.png')
                        print(HTML_IMG_TEMPLATE.format(data, data))
                    return result
