    ...
    def test_call_pro(self):
        ...
        for num in range(len(self.list[2])):
            ...
            for index in range(len(tablelist)):
                ...
                resultsql=conDb2.queryDbReturnMap(self.host,self.port,self.name,self.password,self.table, sql)
                if resultsql=={}:
                        ...
                        for checkname in check.keys():
                            if name == checkname:
                                ...
                                if exceptmap[name]=="notNone":
                                    ...
                                    if str(check[name]).strip() != exceptmap[name] and str(check[name]).strip()+".0" != exceptmap[name] and str(check[name]).strip() != exceptmap[name]+".0":
                                        self.resultcheck = "false"
                                        listhave.append(name+",期望值："+exceptmap[name]+",实际值："+str(check[name]))
                                        biaoshi = "1"
                                    excelishave="1"
