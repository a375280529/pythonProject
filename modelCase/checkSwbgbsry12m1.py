...
#sw_bg_bsry_12m变量测试正常场景
class checkSwbgbsry12m1(unittest.TestCase):
    #执行前调用
    def setUp(self):
    ...
    def test_sw_bg_bsry_12m(self):
        "sw_bg_bsry_12m变量正常场景自动化测试验证"
        ...
        sqlexpect = "select nvl(sum(case when bgxmmc = '%s' and bgrq > to_char(add_months(to_date('%s', 'yyyy-mm-dd'), -12),'yyyy-mm-dd') then 1 else 0 end),0) as sum_sw_bg_bsry_12m " \
        ...
        sbb12mexpect = resultexpect["SUM_SW_BG_BSRY_12M"]

        sqlresult = "select t.sw_bg_bsry_12m from (select sw_bg_bsry_12m from T_JC_ZB where nsrsbh='%s' order by lrsj desc) t where rownum=1" % (
            self.nsrsbh)
        resultreal = conOracle.queryOracleReturnMap(self.url, sqlresult)
        sbb12m = resultreal["SW_BG_BSRY_12M"]
        if sbb12mexpect == sbb12m:
            print("期望值与实际值匹配！", "期望值：" + str(sbb12mexpect), "实际值：" + str(sbb12m))
        else:
            print("期望值与实际值不匹配！", "期望值：" + str(sbb12mexpect), "实际值：" + str(sbb12m))
        self.assertEqual(sbb12mexpect, sbb12m)
