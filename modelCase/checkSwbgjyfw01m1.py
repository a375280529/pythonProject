...
#sw_bg_jyfw_01m变量测试正常场景
class checkSwbgjyfw01m1(unittest.TestCase):
    #执行前调用
    def setUp(self):
        self.url = "vz_bz4/vz_bz4@192.168.85.81:1521/emserver"
        self.nsrsbh = "320803060183187"
        self.spsj = "2015-04-21"
    ...
    def test_sw_bg_jyfw_01m(self):
        "sw_bg_jyfw_01m变量正常场景自动化测试验证"
        ...
        sqlexpect = "select nvl(sum(case when bgxmmc in (%s) and bgrq > to_char(add_months(to_date('%s', 'yyyy-mm-dd'), -1),'yyyy-mm-dd') then 1 else 0 end),0) as sum_sw_bg_jyfw_01m " \
        ...
        sbjf01mexpect = resultexpect["SUM_SW_BG_JYFW_01M"]

        sqlresult = "select t.sw_bg_jyfw_01m from (select sw_bg_jyfw_01m from T_JC_ZB where nsrsbh='%s' order by lrsj desc) t where rownum=1" % (
            self.nsrsbh)
        resultreal = conOracle.queryOracleReturnMap(self.url, sqlresult)
        sbjf01m = resultreal["SW_BG_JYFW_01M"]
        if sbjf01mexpect == sbjf01m:
            print("期望值与实际值匹配！", "期望值：" + str(sbjf01mexpect), "实际值：" + str(sbjf01m))
        else:
            print("期望值与实际值不匹配！", "期望值：" + str(sbjf01mexpect), "实际值：" + str(sbjf01m))
        self.assertEqual(sbjf01mexpect, sbjf01m)
