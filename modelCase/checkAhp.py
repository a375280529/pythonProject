...
#通用存储过程场景测试
class checkAhp(unittest.TestCase):
    #执行前调用
    def setUp(self):
        self.resultcheck = "true"
        self.ini = lh.getIni("loggerAhp.ini")
    ...
    def test_call_pro(self):
        "AHP测试"
        # values = lh.readExcelSheet("F:\\ahp\\ahp深圳中行.xlsx")
        ...
        for coulmnkeys in mapname.keys():
            ...
            if mc["value"] != "None":
                va = mc["value"]
            initvalscore += coulmnkeys + "&" + va + "&" + mc["score"] + "&" + mc["yscqz"] + "&" + mc["zzcqz"] + "&" + \
                            mc["tablename"] + ","
        ...
        for init in initvalscore.split(","):
            ...
            mapyuantable[listnull[0]] = listnull[5]
