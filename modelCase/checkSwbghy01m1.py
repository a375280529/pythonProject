...
#sw_bg_hy_01m变量测试正常场景
class checkSwbghy01m1(unittest.TestCase):
    #执行前调用
    def setUp(self):
    ...
    def test_sw_bg_hy_01m(self):
        "sw_bg_hy_01m变量正常场景自动化测试验证"
        ...
        sqlexpect = "select nvl(sum(case when bgxmmc in (%s) and bgrq > to_char(add_months(to_date('%s', 'yyyy-mm-dd'), -1),'yyyy-mm-dd') then 1 else 0 end),0) as sum_sw_bg_hy_01m " \
        ...
        sbh01mexpect = resultexpect["SUM_SW_BG_HY_01M"]

        sqlresult = "select t.sw_bg_hy_01m from (select sw_bg_hy_01m from T_JC_ZB where nsrsbh='%s' order by lrsj desc) t where rownum=1" % (
            self.nsrsbh)
        resultreal = conOracle.queryOracleReturnMap(self.url, sqlresult)
        sbh01m = resultreal["SW_BG_HY_01M"]
        if sbh01mexpect == sbh01m:
            print("期望值与实际值匹配！", "期望值：" + str(sbh01mexpect), "实际值：" + str(sbh01m))
        else:
            print("期望值与实际值不匹配！", "期望值：" + str(sbh01mexpect), "实际值：" + str(sbh01m))
        self.assertEqual(sbh01mexpect, sbh01m)
