...
#sw_bg_jydz_03m变量测试正常场景
class checkSwbgjydz03m1(unittest.TestCase):
    #执行前调用
    def setUp(self):
    ...
    def test_sw_bg_jydz_03m(self):
        "sw_bg_jydz_03m变量正常场景自动化测试验证"
        ...
        sqlexpect = "select nvl(sum(case when bgxmmc in (%s) and bgrq > to_char(add_months(to_date('%s', 'yyyy-mm-dd'), -3),'yyyy-mm-dd') then 1 else 0 end),0) as sum_sw_bg_jydz_03m " \
        ...
        sbj03mexpect = resultexpect["SUM_SW_BG_JYDZ_03M"]

        sqlresult = "select t.sw_bg_jydz_03m from (select sw_bg_jydz_03m from T_JC_ZB where nsrsbh='%s' order by lrsj desc) t where rownum=1" % (
            self.nsrsbh)
        resultreal = conOracle.queryOracleReturnMap(self.url, sqlresult)
        sbj03m = resultreal["SW_BG_JYDZ_03M"]
        if sbj03mexpect == sbj03m:
            print("期望值与实际值匹配！", "期望值：" + str(sbj03mexpect), "实际值：" + str(sbj03m))
        else:
            print("期望值与实际值不匹配！", "期望值：" + str(sbj03mexpect), "实际值：" + str(sbj03m))
        self.assertEqual(sbj03mexpect, sbj03m)
