...
#sw_bg_fr_24m变量测试正常场景
class checkSwbgfr24m1(unittest.TestCase):
    #执行前调用
    def setUp(self):
    ...
    def test_sw_bg_fr_24m(self):
        "sw_bg_fr_24m变量正常场景自动化测试验证"
        ...
        sqlexpect = "select nvl(sum(case when bgxmmc in (%s) and bgrq > to_char(add_months(to_date('%s', 'yyyy-mm-dd'), -24),'yyyy-mm-dd') then 1 else 0 end),0) as sum_sw_bg_fr_24m " \
        ...
        sbf24mexpect = resultexpect["SUM_SW_BG_FR_24M"]

        sqlresult = "select t.SW_BG_FR_24M from (select SW_BG_FR_24M from T_JC_ZB where nsrsbh='%s' order by lrsj desc) t where rownum=1" % (
            self.nsrsbh)
        resultreal = conOracle.queryOracleReturnMap(self.url, sqlresult)
        sbf24m = resultreal["SW_BG_FR_24M"]
        if sbf24mexpect == sbf24m:
            print("期望值与实际值匹配！", "期望值：" + str(sbf24mexpect), "实际值：" + str(sbf24m))
        else:
            print("期望值与实际值不匹配！", "期望值：" + str(sbf24mexpect), "实际值：" + str(sbf24m))
        self.assertEqual(sbf24mexpect, sbf24m)
