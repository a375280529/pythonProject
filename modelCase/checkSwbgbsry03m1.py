...
#sw_bg_bsry_03m变量测试正常场景
class checkSwbgbsry03m1(unittest.TestCase):
    #执行前调用
    def setUp(self):
    ...
    def test_sw_bg_bsry_03m(self):
        "sw_bg_bsry_03m变量正常场景自动化测试验证"
        ...
        sqlexpect = "select nvl(sum(case when bgxmmc = '%s' and bgrq > to_char(add_months(to_date('%s', 'yyyy-mm-dd'), -3),'yyyy-mm-dd') then 1 else 0 end),0) as sum_sw_bg_bsry_03m " \
        ...
        sbb03mexpect = resultexpect["SUM_SW_BG_BSRY_03M"]

        sqlresult = "select t.sw_bg_bsry_03m from (select sw_bg_bsry_03m from T_JC_ZB where nsrsbh='%s' order by lrsj desc) t where rownum=1" % (
            self.nsrsbh)
        resultreal = conOracle.queryOracleReturnMap(self.url, sqlresult)
        sbb03m = resultreal["SW_BG_BSRY_03M"]
        if sbb03mexpect == sbb03m:
            print("期望值与实际值匹配！", "期望值：" + str(sbb03mexpect), "实际值：" + str(sbb03m))
        else:
            print("期望值与实际值不匹配！", "期望值：" + str(sbb03mexpect), "实际值：" + str(sbb03m))
        self.assertEqual(sbb03mexpect, sbb03m)
